...
//
// ── WHERE A CLAIM LIVES: THE OVERSEER, NOT A CHAIR ────────────────────────────────────────────────────────
// A claim must be answered by ONE party, one request at a time, or two bots asking in the same instant are
// both told yes. The overseer is that party (overseer_brain SECTION 1 decides each request synchronously).
// Corporate HQ is a file per bot with no lock, so a claim table there could double-grant (Architect's choice,
// 2026-09-15: the overseer). The key's prefix names the kind of target and must be on the overseer's
...
// raised the lifecycle ends it). A killed signal skips that release, which overseer_link.releaseAllClaims
...
// module into overseer_link for half of one lifecycle.
function releaseTarget(key) {
  if (key) require('@kernel/overseer_link').releaseClaim(key);
}
