...
| `scripts/` | 9 | **The one-button PowerShell.** `_node.ps1` and `npm.ps1` resolve this machine's toolchain and say which one they picked; the rest raise the cameras, the remote hub and the dashboard. The three fleet launchers are gone — a run is started by `run.js` (2026-09-10). |
